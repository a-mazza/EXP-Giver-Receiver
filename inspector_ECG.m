...
events(end+1) = size(data,1);
    ...
    for i = 1:length(events)-1
        ...
        results = ECG_Analyze(trial.ECG,500);
        f = gcf;
        f.WindowState = 'maximized';
        title(['Sogg ' num2str(trial.Subj(2)) ' Trial ' char(trial.TrialN{2}) trial.Other(2) 'N Picchi' num2str(length(results.RR)+1)  ' Media' num2str(mean(results.RR)) ' Max' num2str(max(results.RR)) ' Min' num2str(min(results.RR))])
