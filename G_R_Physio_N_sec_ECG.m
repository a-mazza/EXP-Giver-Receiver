...
% define measures
measuresECG = table([results.ECG.BPM].', [results.ECG.SDNN].', [results.ECG.RMSSD].', [results.ECG.IBI].', ...
    [results.ECG.pNN50].', [results.ECG.pLF].', [results.ECG.pHF].', [results.ECG.rpLF].', [results.ECG.rpHF].', [results.ECG.LF_HF_ratio].', ...
    'variablenames', {'BPM' 'SDNN', 'RMSSD', 'IBI', 'pNN50', 'pLF', 'pHF', 'rpLF', 'rpHF', 'LF_HF_ratio'});

% define subj number
measuresECG.Subj(:) = dat.Subj(1);

% define trial number
    for i = 1:length(events)-1
        measuresECG.TrialN(i) = dat.TrialN(events(i));
    end

% define conditions
    for i = 1:length(events)-1
        measuresECG.Other(i) = dat.Other(events(i));
    end

    for i = 1:length(events)-1
        measuresECG.Sguardo(i) = dat.Sguardo(events(i));
    end

measuresECG = movevars(measuresECG,11:14, 'before', 1);

MeasuresECG = [MeasuresECG;measuresECG];


clear events results measuresECG
end

%%

clearvars -except MeasuresECG Xstart Xend

save(['Physio_' num2str(Xstart) '_to_' num2str(Xend) '_ECG.mat']);
